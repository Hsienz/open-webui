			...
			for (var [i, containerInfo] of containerInfos.entries()) {
				modelContainers = [
					...modelContainers,
					{
						...
						served_model_name: containerInfo.served_model_name,
	...
	const toggleModelContainerHandler = async (container: ModelContainer) => {
		...
		if (!container.is_active) {
			...
			let data = {
				...
				served_model_name: container.served_model_name,
				...
				<ScrollArea class="h-full py-2">
					<div class={'flex flex-col gap-y-4 text-sm'}>
						{#each modelContainers as container (container.model)}
							<div
								class="flex flex-col border-[1px] rounded-xl min-h-20 px-4 py-2 opacity-50 hover:opacity-100 transition-all duration-300"
							>
								<div class="flex">
								...
								<Collapsible.Root>
									<Collapsible.Trigger class="mx-auto w-full">
										<ChevronUpDown className="size-8 mx-auto" />
									</Collapsible.Trigger>
									<Collapsible.Content>
										<div class="flex flex-col items-center gap-x-2">
											...
											<span class="w-full flex justify-between">
												<div class="w-full flex justify-between">
													<label for={`${container.served_model_name}-served-model-name`}
														>served_model_name</label
													>
													<input
														id={`${container.served_model_name}-served-model-name`}
														name="served-model-name"
														placeholder={container.model}
														disabled={container.is_active}
														bind:value={container.served_model_name}
													/>
												</div>
											</span>
											<span class="w-full flex justify-between">
